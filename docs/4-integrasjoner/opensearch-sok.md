...
> Disse punktene er **ikke løst i dette scopet**, men påvirker kvaliteten på søket. Bør avklares tidlig, helst før oppgave 3.

### 1. Ingen automatisk statusovergang ved utløp

I dag settes ikke treffets domènestatus automatisk når `tilTid` passerer. Et treff med `status=PUBLISERT` forblir `PUBLISERT` selv etter at det er over. Visningsstatus `UTLØPT` avledes derfor beregnet i søketidspunktet, ikke lagret.

Den rette løsningen er en **scheduler i `rekrutteringstreff-api`** som setter faktisk domenestatus (f.eks. en ny `UTLØPT`-status, eller gjenbruk av `FULLFØRT`) når `tilTid` passerer. Da:

- Kan OpenSearch aggregere direkte på `status`-feltet uten tidsbetingelser
- Slipper man Filter Aggregations og sammensatt query-logikk i søke-appen
- Er statusen konsistent på tvers av API, frontend og søk

**Åpent spørsmål:** Skal det opprettes en ny domenestatus `UTLØPT`, eller gjenbrukes `FULLFØRT` (som i dag bare settes manuelt)?

### 2. Terminologigap mellom domenemodell og søkegrensesnitt

Frontend-tabs og filtre bruker brukervennlige termer (`Åpen for søkere`, `Stengt for søkere`, `Utløpt`) som ikke finnes som faktiske statuser i backend. Søkeappen inneholder i dag en eksplisitt mapping mellom disse to begrepsverdener.

Dette skaper friksjon: nye filtre eller statuser må oppdateres på to steder, og det er lett å miste synkroniseringen.

**Mulig løsning:** Enten utvide domenemodellen slik at visningsstatusene faktisk finnes som statuser (se punkt 1), eller formalisere mappingen som en eksplisitt del av søke-appens ansvarsområde med tydelig dokumentasjon og tester – slik at gapet er bevisst og ikke en skjult antagelse.
