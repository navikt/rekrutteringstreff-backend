...
**Fullt dekket i denne planen: kombinert typeahead.** Det er nytt funksjonalitet, ingen legacy-brukere, og det er her risikoen ligger (dynamisk n-token SQL, korrekt populering fra `tmp_*`, kategori-filter, rangering). Her kreves god dekning.

**Ikke utvidet test-dekning for eksisterende typeahead-tabeller (stilling, kompetanse, autorisasjon, godkjenninger, fagdokumentasjon, geografi) som del av denne planen.** De har allerede kjørt i produksjon lenge og har implisitt dekning via at de allerede rensede `tmp_typeahead_*`-tabellene er input til kombinert typeahead — hvis én av dem skulle være tom eller feil, fanges det opp av kombinert-typeahead-testen som leser fra samme kilde.
...
### Konkrete tester for kombinert typeahead

**Repository (`JanzzRepositoryTest`, Testcontainers-basert):**

- `slettOgIndekserTypeaheadArbeidsgiversBehov("tmp_")` populerer fra alle fem `tmp_typeahead_*`-kilder med korrekt `kategori`-verdi per rad.
- Populeringen inkluderer de hardkodede førerkort-radene (antall = 19) med `kategori='FORERKORT'` og `konsept_id=NULL`.
- `overforTypeahead("typeahead_arbeidsgivers_behov")` flytter rader fra `tmp_*` til prod-tabellen.
- `finnTypeaheadArbeidsgiversBehov` med én-token query returnerer riktig rangering: eksakt match først, deretter prefix-match, deretter ordgrense, deretter substring.
- `finnTypeaheadArbeidsgiversBehov` med to-token og tre-token query bruker AND-logikk — alle tokens må matche.
- `finnTypeaheadArbeidsgiversBehov` med `kategorier`-filter returnerer kun rader i de angitte kategoriene.
- `finnTypeaheadArbeidsgiversBehov` med `kategorier=null` returnerer rader fra alle kategorier.
...
- `GET /rest/typeahead/arbeidsgivers-behov?q=a` (1 tegn) returnerer `400`.
- `GET /rest/typeahead/arbeidsgivers-behov?q=sy` returnerer `200` med JSON-array.
- `kategorier=YRKESTITTEL,KOMPETANSE` parses korrekt til `List<BehovKategori>`.
- Ugyldig kategori-verdi gir `400`.
...
- Delegerer til repository og mapper resultatet til `ArbeidsgiversBehov`-DTO uten tap av felter.
