...
class InnleggService(
    ...
    fun opprettInnlegg(treffId: TreffId, dto: OpprettInnleggRequestDto, navIdent: String): Innlegg {
        ...
        if (rekrutteringstreff.status == RekrutteringstreffStatus.UTKAST) {
            ...
            logger.info("Prøver å opprette innlegg for treff $treffId som er i status UTKAST, og har ${innleggForTreff.size} innlegg")
            if (innleggForTreff.isNotEmpty()) {
                logger.warn("Innlegg finnes allerede for treff $treffId - overskriver eksisterende innlegg")
