...
""".trimIndent()
    ...
    """.trimIndent()
        ...
        val oppmøteHendelser = hendelser.filter { it.hendelsestype == JobbsøkerHendelsestype.MØTT_OPP }
    ...
    @Test
    fun registrerIkkeOppmøteForJobbsøkere() {
        val token = authServer.lagToken(authPort, navIdent = "A123456")
        val treffId = db.opprettRekrutteringstreffIDatabase()
        val fnr1 = Fødselsnummer("12312312312")
        val fnr2 = Fødselsnummer("45645645645")
    ...
    """.trimIndent()
        ...
        val (_, r, res) = Fuel.post("http://localhost:$appPort/api/rekrutteringstreff/$treffId/jobbsoker/registrer-ikke-oppmote")
            .body(requestBody)
            .header("Content-Type", "application/json")
            .header("Authorization", "Bearer ${token.serialize()}")
            .responseString()
        ...
        val ikkeOppmøteHendelser = hendelser.filter { it.hendelsestype == JobbsøkerHendelsestype.IKKE_MØTT_OPP }
        assertThat(ikkeOppmøteHendelser).hasSize(2)
        ikkeOppmøteHendelser.forEach {
            assertThat(it.opprettetAvAktørType).isEqualTo(AktørType.ARRANGØR)
            assertThat(it.aktørIdentifikasjon).isEqualTo("A123456")
        }

        val registrertOmmmøteFødselsnumre =
            ikkeOppmøteHendelser.map { db.hentFødselsnummerForJobbsøkerHendelse(it.id)}
